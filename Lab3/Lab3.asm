...
# 		chun38
# 		4 November 2022
# Assignment: Lab3: RARS Introduction
...
# Notes: This program is intended to be run using RARS
	...
	#................ pseudo code here..........................................................#
	# t1: counter (adds one for each action until equal to user input)
	# t0: user input (n)
	# t2: small loop counter (check with counter)
	# big loop adds /n to the end
	# small loop adds the stars and blank spaces 
		# needs to be able to check which line it's drawing (using the counter t0)
		# done with small loop then go to big loop
	# needs to check if t1 == t0 
		# if counter still smaller than user input, add one to t1 and loop again
	# done with big loop then exit the program
	# need to use branching for loops
...
smallLoop:
	beq t1, t0, drawStarLine

	beq t2, zero, drawStar
	addi t2, t2, 1
	beq t2, t1, drawStar
	print_str(blackspace)
	#display ' ' on screen
	write_to_buffer(0x20)
	#0x20 is hex asci code for ' '
	blt t2, t1, smallLoop
